#
#Author : Manimaran
#To Enable IPv6 Member DNS for all the members
#
use strict;
use warnings;
use Infoblox;
...
foreach (@retrieved_objs) {
    ...
    $session->modify($_) or $session->status_detail("The IPv6 DNS service cannot be enabled on LAN1 because LAN1 does not have an IPv6 address configured") or die("Enable Lan Ipv6 Interface failed: ", $session->status_code() . ":" . $session->status_detail());
